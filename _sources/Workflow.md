...
- The actual data files are in subfolders of `scalar` / `vector`
    ...
    - Example: Nitrate data for 2016 has folder name `~/ooi/sb/ooinet/scalar/2016_nitrate`
...
- Several approaches to post-processing are possible, numbered 01, 02, ...
    - This operates on sharded (redux) data with results > `~/ooi/<site>/postproc/pp<NN>`
- Data analysis methods operate on `redux` or `postproc` versions of the data
    - Results are written to `~/ooi/<site>/analysis/<subfolder>`
    - So far there are two: `analysis/clustering` and `analysis/sga`
        - `sga` refers to spectral graph analysis
- Profile Indices provided by the RCA data team reside in `~/ooi/<site>/profileIndices`
- Visualizations (typically pngs produced from charts) reside in `~/ooi/<site>/visualizations/`
...
- (1) Automated: Data download (script)
    ...
    - Data > localhost `~/ooi/<site>/ooinet/{scalar or vector}/<year_instrument>/fnm.nc`
    - Additional step: Scan for and delete superfluous (time-overlap) data files
- (2) Automated: Sharding from above source/raw files to `~/ooi/<site>/redux/2018/etcetera`
...
- (3) Automated: Post-processing
    ...
    - Post-processing results written to folders `~/ooi/<site>/postproc/pp<NN>`
...
- Files are to be downloaded to the corresponging localhost destination folder
    - The base localhost folder location is `~/ooi/sb/ooinet/scalar/<yyyy>_<instrument>`
...
Shard folder name example: `~/ooi/sb/redux/2018`
...
**Prefix structure mirrors the per-site localhost layout** (`<site>` = `sb`/`oo`/`ab`).
See `DataOps.md` for the authoritative S3 layout, sync/verify/restore commands, and
bucket policy. Summary:
```
s3://s3ooi/<site>/redux/<yyyy>/...             (sharded profiles)
s3://s3ooi/<site>/postproc/<pp>/<yyyy>/...     (post-processed subsets)
s3://s3ooi/ooinet/rca/...                      (raw source archive; legacy keys, not re-keyed)
```

**Sync commands** (see `DataOps.md` for the full set):
```bash
